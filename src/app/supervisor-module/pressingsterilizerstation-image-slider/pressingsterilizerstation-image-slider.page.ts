...
export class PressingsterilizerstationImageSliderPage implements OnInit {
  ...
  constructor(
    ...
    if (this.fromscreen == "Press") {
    ...
    } else if (this.fromscreen == "Sterilisation") {
      ...
      this.fromscreen == "Grading" ||
      this.fromscreen == "GradingReport"
    ) {
    ...
    } else if (this.fromscreen == "FFBSupplierReport") {
      let ffbsupplierparams = navParams.get("ffbsupplieritem");

      if (ffbsupplierparams.length > 0) {
        this.ffbdespatchimages = ffbsupplierparams.split("~");
      }

      for (let i = 0; i < this.ffbdespatchimages.length; i++) {
        let eachitem = this.ffbdespatchimages[i];
        let eachreq = {
          image: eachitem,
          title: this.translate.instant("IMAGESLIDER.image") + (i + 1),
        };
    ...
    } else if (this.fromscreen == "Ramp") {
      let rampparams = navParams.get("rampitem");

      if (rampparams.length > 0) {
        this.rampdespatchimages = rampparams.split("~");
      }

      for (let i = 0; i < this.rampdespatchimages.length; i++) {
        let eachitem = this.rampdespatchimages[i];
        let eachreq = {
          image: eachitem,
          title: this.translate.instant("IMAGESLIDER.image") + (i + 1),
        };
